...
RUN apk add --no-cache git python3 ca-certificates nmap iputils ffmpeg mariadb-client tini && \
    chmod u+s /bin/ping && \
    addgroup -g ${GUID} hass && \
    adduser -h /config -D -G hass -s /bin/sh -u ${UID} hass && \
    pip3 install --upgrade --no-cache-dir pip && \
    apk add --no-cache --virtual=build-dependencies build-base linux-headers python3-dev libffi-dev libressl-dev libxml2-dev libxslt-dev mariadb-connector-c-dev tzdata && \
